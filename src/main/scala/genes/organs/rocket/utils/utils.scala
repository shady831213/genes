...
package object utils {
  ...
  implicit class SeqMemToAugmentedSeqMem[T <: Data](val x: SyncReadMem[T]) extends AnyVal {
  ...
  implicit class UIntToAugmentedUInt(val x: UInt) extends AnyVal {
    ...
    def andNot(y: UInt): UInt = x & (~WireDefault(t = x, init = y)).asUInt()
  ...
  def UIntToOH1(x: UInt, width: Int): UInt = (~(1.S(width).asUInt << x) (width - 1, 0)).asUInt()
